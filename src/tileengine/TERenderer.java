...
public class TERenderer {
    ...
    private int xOffset;
    private int yOffset;
    public ArrayList<Integer> grassRotations = new ArrayList<>();
        ...
        for (int x = 0; x < numXTiles; x += 1) {
            for (int y = 0; y < numYTiles; y += 1) {
                if (world[x][y] == null) {
                    ...
                    if (grassRotations.size() != World.numGrasses) {
                        int deg = new Random().nextInt(4);
                        grassRotations.add(deg);
                        world[x][y].draw(x + xOffset, y + yOffset, deg * 90);
                    } else {
                        int deg = grassRotations.get(i % World.numGrasses);
                        i++;
                        world[x][y].draw(x + xOffset, y + yOffset, deg * 90);
                    }
                } else {
